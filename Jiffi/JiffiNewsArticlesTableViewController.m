...
- (void)viewDidLoad
{
    ...
    self.articlesDictionary = [[NSDictionary alloc] initWithObjectsAndKeys:@"Hello world. First article", @"Article 1", nil];
    self.articlesArray = @[@"Article 1"];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // Configure the cell...
    NSString *articleName = self.articlesArray[indexPath.row];
    NSLog(@"%@", self.articlesDictionary);
    NSString *articleContents = [self.articlesDictionary objectForKey:articleName];
...
- (void) prepareForSpritz:(ViewController *)vc toSendInfo:(NSString *)info {
    vc.articleTitle = info;
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if([segue.identifier isEqualToString:@"newsSpritz"]){
        if([[segue destinationViewController] isKindOfClass:[ViewController class]]) {
            UITableViewCell *cell = (UITableViewCell*)sender;
            NSString *title = cell.textLabel.text;
            [self prepareForSpritz:segue.destinationViewController toSendInfo:title];
        }
